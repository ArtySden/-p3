function TareaForm({ onGuardar }) {
  ...
  return (
    <form onSubmit={enviar} className="bg-white rounded-3xl shadow-xl responsive-card border border-gray-200" noValidate>
      <h3 className="text-xl sm:text-2xl font-black text-gray-950">Agregar tarea</h3>
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div className="md:col-span-2 xl:col-span-1">
          ...
          <input id="archivoTarea" type="file" onChange={(e) => setArchivo(e.target.files[0])} className="focus-ring mt-2 w-full border border-gray-300 rounded-xl px-4 py-2.5 file:mr-3 file:rounded-lg file:border-0 file:bg-purple-100 file:px-3 file:py-2 file:font-bold file:text-purple-800" />
        </div>
      </div>
      {error && <p className="mt-4 text-sm font-bold text-red-700" role="alert">{error}</p>}
      <button type="submit" className="focus-ring touch-target mt-5 bg-purple-700 hover:bg-purple-800 text-white px-6 py-3 rounded-xl font-black transition mobile-full sm:w-auto">Guardar tarea</button>
    </form>
  );
}
    ...
    <article className="card-hover bg-white border border-gray-200 rounded-3xl p-5 shadow-lg">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h3 className={`mt-1 text-xl font-black break-words ${tarea.estado === "completada" ? "line-through text-gray-500" : "text-gray-950"}`}>{tarea.favorito ? "★ " : "☆ "}{tarea.titulo}</h3>
          <p className="mt-2 text-gray-600">Entrega: {tarea.fecha}</p>
          <p className="mt-1 text-sm text-gray-500 break-words">Archivo: {tarea.archivo ? <a className="text-purple-700 font-bold underline" href={tarea.archivo} target="_blank" rel="noreferrer">{tarea.nombreArchivo}</a> : tarea.nombreArchivo}</p>
        </div>
        <button aria-label="Marcar tarea como favorita" onClick={() => onFavorito(tarea.id)} className="focus-ring touch-target shrink-0 text-2xl rounded-xl px-3 py-2 hover:bg-purple-50">{tarea.favorito ? "★" : "☆"}</button>
      </div>
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <button onClick={() => onEstado(tarea.id, "completada")} className="focus-ring touch-target bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-xl font-bold transition">Completada</button>
        <button onClick={() => onEstado(tarea.id, "pendiente")} className="focus-ring touch-target bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-xl font-bold transition">Pendiente</button>
        <button onClick={() => onEliminar(tarea.id)} className="focus-ring touch-target bg-gray-900 hover:bg-black text-white px-4 py-2 rounded-xl font-bold transition">Eliminar</button>
      </div>
    </article>
  );
}
    ...
    <section className="page-shell fade-in">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          ...
          <h2 className="responsive-subtitle font-black text-gray-950">Tareas universitarias</h2>
      ...
      <div className="mt-8 grid gap-5 xl:grid-cols-2">
        {tareas.length === 0 ? (
          <div className="bg-white rounded-3xl p-6 sm:p-8 shadow text-center xl:col-span-2">
